...
export function DoshaCard({ dosha, defaultExpanded = false }: DoshaCardProps) {
  ...
  const borderColor = dosha.severity ? styles.border : tw(0.08)
        ...
        <div className="flex-1 min-w-0">
          ...
          {!expanded && meaning && (
            <p className="text-xs truncate pr-2" style={{ color: 'var(--text3)' }}>
              ...
              {meaning.whatItMeans.length > 100 ? '…' : ''}
            </p>
          )}
        </div>
